...
fun RectificationScreen(navController: NavController, vm: RectificationScreenViewModel = hiltViewModel()) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Birth Time Rectification", style = MaterialTheme.typography.titleLarge.copy(color = BrahmGold)) },
                navigationIcon = {
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            when {
                ...
                result != null -> RectificationScreenContent(result!!)
                else -> RectificationScreenInputForm { params -> vm.submit(params) }
            }
        }
    }
}
