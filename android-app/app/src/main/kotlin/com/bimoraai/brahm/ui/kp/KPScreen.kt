...
fun KPScreen(navController: NavController, vm: KPScreenViewModel = hiltViewModel()) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("KP System Chart", style = MaterialTheme.typography.titleLarge.copy(color = BrahmGold)) },
                navigationIcon = {
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            when {
                ...
                result != null -> KPScreenContent(result!!)
                else -> KPScreenInputForm { params -> vm.submit(params) }
            }
        }
    }
}
