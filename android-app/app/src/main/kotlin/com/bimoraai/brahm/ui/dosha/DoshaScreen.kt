...
fun DoshaScreen(navController: NavController, vm: DoshaScreenViewModel = hiltViewModel()) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Dosha Analysis", style = MaterialTheme.typography.titleLarge.copy(color = BrahmGold)) },
                navigationIcon = {
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            when {
                ...
                result != null -> DoshaScreenContent(result!!)
                else -> DoshaScreenInputForm { params -> vm.submit(params) }
            }
        }
    }
}
