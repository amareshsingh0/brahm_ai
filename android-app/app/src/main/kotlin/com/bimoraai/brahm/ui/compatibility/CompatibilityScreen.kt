...
fun CompatibilityScreen(navController: NavController, vm: CompatibilityScreenViewModel = hiltViewModel()) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Kundali Matching", style = MaterialTheme.typography.titleLarge.copy(color = BrahmGold)) },
                navigationIcon = {
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            when {
                ...
                result != null -> CompatibilityScreenContent(result!!)
                else -> CompatibilityScreenInputForm { params -> vm.submit(params) }
            }
        }
    }
}
