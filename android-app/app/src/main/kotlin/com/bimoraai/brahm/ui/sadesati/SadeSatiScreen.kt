...
fun SadeSatiScreen(navController: NavController, vm: SadeSatiScreenViewModel = hiltViewModel()) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Saturn Sade Sati", style = MaterialTheme.typography.titleLarge.copy(color = BrahmGold)) },
                navigationIcon = {
        ...
        Box(Modifier.padding(padding).fillMaxSize()) {
            when {
                ...
                result != null -> SadeSatiScreenContent(result!!)
                else -> SadeSatiScreenInputForm { params -> vm.submit(params) }
            }
        }
    }
}
